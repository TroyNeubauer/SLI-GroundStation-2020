...
class GPSTime
{
  ...
  static NeoGPS::neo_clock_t s_start_of_week;
...
public:
    ...
    static void start_of_week( NeoGPS::time_t & now )
      {
        now.set_day();
        s_start_of_week =
          (NeoGPS::neo_clock_t) now  -
          (NeoGPS::neo_clock_t) ((((now.day-1  ) * 24L +
                                now.hours  ) * 60L + 
                                now.minutes) * 60L +
                                now.seconds);
      }

    static NeoGPS::neo_clock_t start_of_week()
    {
     ...
     */
    static NeoGPS::neo_clock_t TOW_to_UTC( uint32_t time_of_week )
      { return (NeoGPS::neo_clock_t)
...
//trace << PSTR("from_TOWms(") << time_of_week_ms << PSTR("), sow = ") << start_of_week() << PSTR(", leap = ") << leap_seconds << endl;
      ...
      if (ok) {
        NeoGPS::neo_clock_t tow_s = time_of_week_ms/1000UL;
        dt = TOW_to_UTC( tow_s ); 
